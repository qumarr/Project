...
class Employee extends Admin_Controller {
    ...
    public function save_employee($id = NULL) {
        ...
        $user_name=$ldata['user_name'] = $data['employment_id'];
        $pass=rand();
        $ldata['password'] = $this->hash($pass);
        ...
        if (!empty($check_existing_data)) {
            ...
            $this->employee_model->save($ldata);
             //sending mail of user details 
        $headers = "From: " .$email;
        $headers .= "Content-Type: text/html; charset=ISO-8859-1\r\n";
        $admin_email = $data['email'];
        $email = 'DoNotReply@thehrm.cf';
        $subject = 'New Employee Account Registration Details ';
        $comment = " Hello $f_name  $l_name ,\n New account has been registered with the following Details \n
        login_userid : $user_name and password: $pass \n
